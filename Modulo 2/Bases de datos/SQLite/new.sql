...
-- Tabla sin los dos datos creados 
...
-- Agregamos columnas después de crear la tabla 
ALTER TABLE facturas ADD COLUMN telefono TEXT;
ALTER TABLE facturas ADD COLUMN codigo_cajero TEXT DEFAULT 'N/A';
...
-- Obtener todas las compras de un producto por ID
-- Une detalle_factura con productos
SELECT df.*
FROM detalle_factura df
JOIN productos p ON df.codigo_producto = p.codigo_producto
WHERE p.codigo_producto = 'P001';

-- Total comprado por producto (agrupado)
-- SUM suma todas las cantidades compradas
SELECT codigo_producto, SUM(cantidad) AS total_comprado
FROM detalle_factura
GROUP BY codigo_producto;

-- Facturas de un mismo comprador
SELECT *
FROM facturas
WHERE correo_comprador = 'cliente@email.com';

-- Facturas ordenadas por monto total DESC
SELECT *
FROM facturas
